...
Run_trimmomatic=YES
# 4
assembler_used=spades # don't change this unless you want to used a different assembler, if this is the case, then also change "Run_metaSPAdes" to "NO"
contig_name_prefix=NODE_ # don't change this unless you want to used a different assembler, if this is the case, then also change "Run_metaSPAdes" to "NO"
Run_metaSPAdes=YES
# 5
...
WorkingPATH=/home/Dataanalysis/Curations/Metagenomic_SE/Date
...
SRA_files_output_directory=/home/Dataanalysis/raw_metagenomic_reads
...
metaSPAdes_output_directory=${Pipeline_output_directory}/metaSPAdes
mkdir ${metaSPAdes_output_directory}
...
#When changing the file names of the output scaffolds of the assembler. e.g. metaSPAdes will label the file "scaffolds.fasta"
assembler_fasta_PREFIX=scaffolds

InputPath_metaSPAdes=${Trimmomatic_output_directory}  # Change this only if you do not want to use Trimmomatic.

# You can change the extensions for the inputs of metaSPAdes here to your preference. If you are using Trimmomatic, the pipeline will automatically adjust the extensions of the Trimmomatic outputs to fit your choice.

InputFileExtension_metaSPAdes=.fastq.qtrim.fastq #Forward reads or reaction 1 reads - change "fastq" to "fasta" if you are using "fasta" inputs for Trimmomatic     
...
# You can edit this in the pipeline to your preference but this should be fine for RNAseq or DNAseq data - Based off Trinity v2.11.0
...
Project_collection=BSF_SE_metagenomes_by_date
...
echo -e "#----------------------------------------------------------------------#\n#                   3. QC and trimming of reads                        #\n#----------------------------------------------------------------------#\n Project is ${ProjectName}" | tee -a ${Pipeline_Logfile} ${Commands_check_log}
   ...
   then 
      ...
      do 
         ...
         mv ${Trimmomatic_output_directory}/${Input}${InputFileExtension_metaSPAdes} ${Trimmomatic_output_directory}/${ProjectName}/${Input}${InputFileExtension_metaSPAdes}
...
echo -e "#----------------------------------------------------------------------#\n#                      4. Assembly of contigs                          #\n#----------------------------------------------------------------------#\n Project is ${ProjectName}" | tee -a ${Pipeline_Logfile} ${Commands_check_log}
   ...
   if [ ${Run_metaSPAdes} = YES ]
   then 
      echo "Script will run metaSPAdes" | tee -a ${Pipeline_Logfile} ${Commands_check_log}
      ${D_And_T} >> ${Pipeline_Logfile}

      mkdir ${metaSPAdes_output_directory}/${ProjectName}

      # To create a string (horizontal list) which has all the sample names with spaces inbetween each name 
      # if you want to do all of this in one line and not have any file remaining afterwards
      InputShortName=($(sed -e "s+${InputFileExtension_metaSPAdes}++g" <(echo $(find ${InputPath_metaSPAdes}/${ProjectName} -type f -name "*${InputFileExtension_metaSPAdes}" -printf "%f "))))
   
      for Input in ${InputShortName[*]} 
      do 
         echo "###Input sample for metaSPAdes is \"${Input}\"" >> ${Pipeline_Logfile}
         ${D_And_T} >> ${Pipeline_Logfile}
         ##metaSPAdes
         echo "##metaSPAdes" >> ${Pipeline_Logfile}
         #metaSPAdes command
         echo "${SPAdesPROG} -m ${Amount_of_RAM} -t ${Num_threads} -s ${InputPath_metaSPAdes}/${ProjectName}/${Input}${InputFileExtension_metaSPAdes} -o ${metaSPAdes_output_directory}/${ProjectName}/${Input}_metaSPAdesoutput" >> ${Commands_check_log}

         ${SPAdesPROG} -m ${Amount_of_RAM} -t ${Num_threads} -s ${InputPath_metaSPAdes}/${ProjectName}/${Input}${InputFileExtension_metaSPAdes} -o ${metaSPAdes_output_directory}/${ProjectName}/${Input}_metaSPAdesoutput
      ${D_And_T} >> ${Pipeline_Logfile}
      done
   else
      echo "Script will NOT run metaSPAdes" | tee -a ${Pipeline_Logfile} ${Commands_check_log}
   fi
...
echo -e "#----------------------------------------------------------------------#\n#                  5. Renaming the contig names                        #\n#----------------------------------------------------------------------#\n Project is ${ProjectName}" | tee -a ${Pipeline_Logfile} ${Commands_check_log}
   ...
   then 
      ...
      do 
         ...
         echo "cp ${metaSPAdes_output_directory}/${ProjectName}/${Input}_metaSPAdesoutput/${assembler_fasta_PREFIX}${Diamond_InputFileExtension} ${labelled}/${assembler_fasta_PREFIX}_${Input}${Diamond_InputFileExtension}" >> ${Commands_check_log}

         cp ${metaSPAdes_output_directory}/${ProjectName}/${Input}_metaSPAdesoutput/${assembler_fasta_PREFIX}${Diamond_InputFileExtension} ${labelled}/${assembler_fasta_PREFIX}_${Input}${Diamond_InputFileExtension}
